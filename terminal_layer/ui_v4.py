...
def render_summary_large(ps: PipelineState, scroll: ScrollState,
    ...
    for t in ["query", "channel", "playlist", "direct"]:
        ...
        disc_text.append_text(_bar(n, maxn, C_CYAN, "color(23)"))
    ...
    # header row
    dl_text.append(f"  {'VIDEO':<32}", style=f"dim {color}")
    dl_text.append(f"{'SIZE':>8}",     style=f"dim {color}")
    dl_text.append(f"  {'SOURCE':<12}",style=f"dim {color}")
    dl_text.append(f"  STATUS\n",      style=f"dim {color}")
    dl_text.append(f"  {'─'*66}\n",    style=f"dim {color}")

    # scrollable rows
    fixed_lines = 22  # cards(4) + breakdown(7) + dl header(3) + output(4) + padding
    ...
    for c in visible:
        ...
        dl_text.append(f"  {name_s:<32}", style=row_s)
        dl_text.append(f"{size_s:>8}",    style=f"dim {color}")
        dl_text.append(f"  {src_s:<12}",  style="dim white")
    ...
    return Panel(
        Group(cards_table, disc_text, dl_text, out_text),
