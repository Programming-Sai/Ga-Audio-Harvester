...
def render_summary_large(ps: PipelineState, scroll: ScrollState,
    ...
    # ── STAT CARDS ─────────────────────────────
    # render as a single line row of mini-cards using box-drawing
    body.append("\n")
    cards = [
        ("DOWNLOADED", str(dl.done),       f"of {dl.total} resolved", C_GREEN),
        ("ERRORS",     str(dl.errors),     "age-restr / unavail",     C_RED),
        ("TOTAL SIZE", f"{dl.total_mb/1024:.1f} GB" if dl.total_mb >= 1024
                       else f"{dl.total_mb} MB",
        ...
        ("RETRIES",    str(rs.retries),    "0 permanent failures",    C_MAGENTA),
    ]
    ...
    for i, (lbl, val, sub, col) in enumerate(cards):
        w = 18
        body.append("┌" + "─"*w + ("┐" if i == len(cards)-1 else "┬"), style="dim white")
    body.append("\n")
    ...
    for lbl, val, sub, col in cards:
        ...
        body.append(f" {lbl:<16} ", style=f"dim {col}")
    body.append("│\n", style="dim white")
    ...
    for lbl, val, sub, col in cards:
        body.append("│", style="dim white")
        body.append(f" {val:<16} ", style=f"bold {col}")
    body.append("│\n", style="dim white")
    ...
    for lbl, val, sub, col in cards:
        body.append("│", style="dim white")
        s = sub[:16]
        body.append(f" {s:<16} ", style=f"dim {col}")
    body.append("│\n", style="dim white")
    ...
    for t in ["query", "channel", "playlist", "direct"]:
        ...
        body.append(f"  {label}", style=f"dim {C_CYAN}")
        body.append(f" {str(n).rjust(3)} ", style=f"bold {C_CYAN}")
        body.append_text(_bar(n, maxn, C_CYAN, "color(23)"))
        body.append(f"  {note}\n", style=f"dim {C_CYAN}")

    body.append(f"  {'─'*68}\n", style=f"dim {C_CYAN}")
    body.append(f"  Total resolved: ", style=f"dim {C_CYAN}")
    body.append(f"{total}", style=f"bold {C_CYAN}")
    body.append(f"   skipped: ", style=f"dim {C_CYAN}")
    body.append(f"{skipped}", style=f"bold {C_CYAN}")
    body.append(f"   parse errors: ", style=f"dim {C_CYAN}")
    body.append(f"{ds.errors}\n", style=f"bold {C_CYAN}")
    ...
    # header
    body.append("  ")
    body.append(f"{'VIDEO':<32}", style=f"dim {C_GREEN}")
    body.append(f"{'SIZE':>8}",   style=f"dim {C_GREEN}")
    body.append(f"  {'SOURCE':<12}", style=f"dim {C_GREEN}")
    body.append(f"  {'STATUS'}\n",   style=f"dim {C_GREEN}")
    body.append(f"  {'─'*68}\n",     style=f"dim {C_GREEN}")

    # scrollable results
    fixed_above = 28   # approx lines used by cards + breakdown + headers
    log_height  = max(3, height - fixed_above - 8)

    with dl.lock:
        completed = list(dl.completed)

    # build log-style entries for scroll support
    result_lines = []
    for c in completed:
    ...
    if sc.following or sc.offset == 0:
        visible = items[-log_height:] if len(items) > log_height else items
    else:
        end     = max(0, len(items) - sc.offset)
        start   = max(0, end - log_height)
        visible = items[start:end]

    for c in visible:
        name_s = (c.name[:30] + "…") if len(c.name) > 31 else c.name
        size_s = f"{c.size_mb}MB" if c.size_mb else "—"
        src_s  = c.source[:11]

        if c.status == "OK":
            st_style = f"bold {C_GREEN}"
        elif c.status == "ERR":
            st_style = f"bold {C_RED}"
        else:
            st_style = "dim white"

        body.append(f"  {name_s:<32}", style=C_WHITE if c.status == "OK" else C_DIM)
        body.append(f"{size_s:>8}",    style=f"dim {C_GREEN}")
        body.append(f"  {src_s:<12}", style="dim white")
        body.append(f"  {c.status}\n", style=st_style)

    if not scroll.following and len(result_lines) > log_height:
        body.append(f"  ↑ scrolled  (↓ to follow)\n", style=f"dim {C_GREEN}")

    # ── OUTPUT PATHS ───────────────────────────
    body.append(f"\n  OUTPUT\n", style=f"dim {C_GREEN}")
    body.append(f"  {'─'*68}\n", style=f"dim {C_GREEN}")
    body.append("  Directory  ", style=f"dim {C_GREEN}")
    body.append(f"{os.path.abspath(ps.output_dir)}\n", style=C_GREEN)
    fname = time.strftime("run_%Y%m%d_%H%M%S.json", time.localtime(ps.start_time))
    body.append("  Summary    ", style=f"dim {C_GREEN}")
    body.append(f"{fname}\n", style=C_GREEN)

    return Panel(
        body,
        title=Text.assemble(("  RUN SUMMARY  ", f"bold {C_GREEN}")),
        title_align="left",
        subtitle=Text.assemble((
            f" run #0001  ─  completed {time.strftime('%H:%M:%S', time.localtime(ps.end_time or time.time()))} ",
            f"dim {C_GREEN}")),
...
def build_layout(ps: PipelineState, console: Console) -> Layout:
    ...
    phase_colors = {PipelinePhase.DISCOVERY: C_CYAN,
                    ...
                    PipelinePhase.DONE:      C_GREEN}
    pc = phase_colors[ps.phase]

    if ps.phase == PipelinePhase.DONE:
        topbar.append(" ✓ COMPLETE ", style=f"bold {C_GREEN}")
    else:
